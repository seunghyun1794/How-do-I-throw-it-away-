...
function normalizeLocationInput(value, fallback = '위치 정보 없음') {
  let source = value;
  if (source && typeof source === 'object') source = source.name;

  const text = cleanText(source, '');
  if (!text || text === '[object Object]' || text === '찾는 중...' || text === '위치 미확인') {
    return fallback;
  }
...
async function analyzeImage({ imageBase64, mimeType, location = '현재 위치' }) {
  ...
  const safeLocation = normalizeLocationInput(location, '위치 정보 없음');
